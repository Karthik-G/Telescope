...
PostsListController = FastRender.RouteController.extend({
  ...
  onBeforeAction: function () {
    ...
    if(this.params._userId) {
      this._terms.query = this.params._userId;//Session.get("searchQuery");this._terms.query = Session.get("searchQuery");
    }
  ...
  data: function () {
    ...
    if(this._terms.query)
      {
        var x=0;
       Meteor.users.find({username:this._terms.query}).forEach(function (user) {
         x = user._id;
       });
       this._terms.query=x;
        // console.log("foundUserId "+this._terms.query);
      }
    console.log("Inside search "+this._terms.query);
    var parameters = getPostsParameters(this._terms),
     ...
     if(this._terms.query!=undefined)
     {
          //  postsCount = Posts.find({userId:this._terms.query}).count();
    ...
    if(this._terms.query!=undefined)
     {
            //posts = Posts.find({userId:this._terms.query});
...
PostPageController = FastRender.RouteController.extend({
  ...
  post: function() {
   // var post=Posts.findOne(this.params._id);
    ...
    return  Posts.findOne(this.params._id);//Posts.find({userId:post.userId}).fetch();
  },
  ...
  data: function() {
       var singlePost=this.post();
       var url= "/q/"+singlePost.userId;
       Session.set('firstPost', singlePost);
       Router.go(url);
    //osts.findOne(this.params._id)
    //return
  }
});
...
UserPageController = FastRender.RouteController.extend({
  ...
  data: function() {
    ...
    if(typeof findById !== "undefined"){
      // redirect to slug-based template: 'postDiscuss',
...
this.route('apiPosts', {
  ...
  action: function() {
	...
	this.response.write("jsonCallback(" + JSON.stringify(data) + ")");
	this.response.end();
  }
});
...
this.route('apiPosts', {
  where: 'server',
  path: '/xapi/listsForUser',
  action: function() {
		console.log("UserId is there as  "+ this.params.userId);
		console.log("Url is there as  "+ this.params.url);

	var data = null;

  if(this.params.url!=undefined) {
	data = Posts.find({userId:this.params.userId,url:this.params.url}).fetch();
  }
  else {
	data = Posts.find({userId:this.params.userId}).fetch();
  }
...
this.route('apiPosts', {
  where: 'server',
  path: '/xapi/getUserForGDUserName',
  action: function() {
        console.log("Username is there as  "+ this.params.username);
    var data = Meteor.users.find({username:this.params.username}).fetch();
...
this.route('apiPosts', {
  where: 'server',
  path: '/xapi/listsForUrl',
  action: function() {
    console.log("Url is there as  "+ this.params.url);
    this.params.url="http://localhost:8080/facebook";
    var randomPostsFromMultipleLists = Posts.find({url:this.params.url}).fetch();
    var arrayOfFirstPosts= new Array;
        console.log("randomPostsFromMultipleLists  "
          + JSON.stringify(randomPostsFromMultipleLists));

 console.log("randomPostsFromMultipleLists  length " +randomPostsFromMultipleLists.length);

 var arrayOfUsers= new Array;
for (var i = randomPostsFromMultipleLists.length - 1; i >= 0; i--) {

     console.log("randomPostsFromMultipleLists loop iteration "+i);
      var user = Meteor.users.findOne(randomPostsFromMultipleLists[i].userId);
     console.log("Username is is there as  "+  user.username);
      //var user = Meteor.users.findOne(data[i].userId);
      //randomPostsFromMultipleLists[i].author=user.username;
      
     var len = arrayOfUsers.length;
    var continueflag=false;
    for(var ix = 0 ; ix < len;ix++)
    {
        if(arrayOfUsers[ix] == user.username){continueflag=true; break;}
    }
   if(continueflag) continue;

      arrayOfUsers.push( user.username);

      var allPostsForThatUser=Posts.find({userId:randomPostsFromMultipleLists[i].userId}).fetch();
   
       var tempPost=allPostsForThatUser[0];
        var user = Meteor.users.findOne(tempPost.userId);
         tempPost.author=user.username;

       for (var j = allPostsForThatUser.length - 1; j >= 1; j--) {
        if(new Date(allPostsForThatUser[j].createdAt).getTime() < new Date(tempPost.createdAt.getTime()))
        {
          tempPost=allPostsForThatUser[j];
           user = Meteor.users.findOne(tempPost.userId);
           tempPost.author=user.username;

        }
       };
      console.log(" post loop iteration "+i);

       arrayOfFirstPosts.push(tempPost);
};
/*
  ...
  */
  console.log("Returned arrayOfFirstPosts length "+arrayOfFirstPosts.length);
  console.log("JSON returned "+JSON.stringify(arrayOfFirstPosts));

	this.response.writeHead(200, {'Content-Type': 'text/javascript'});
	this.response.write("jsonCallback(" + JSON.stringify(arrayOfFirstPosts) + ")");
	this.response.end();
  }
});
...
this.route('apiPosts', {
  where: 'server',
  path: '/xapi/listCreate',
  action: function() {
        console.log("url param is there as  "+ this.params.url );
        console.log("userId param is there as  "+ this.params.userId );
        console.log("title param is there as  "+ this.params.title);
        console.log("status param is there as  "+ this.params.status );
       // console.log("Params is there as  "+ this.params.categories );

       var post = {
          url : this.params.url,
          userId : this.params.userId,
          title : this.params.title.replace(/\+/g," "),
          status: this.params.status,
          body: this.params.body.replace(/\+/g," ")
       };
        ...
        /*Meteor.call('post', {data: this.params}, function(error, post) {
        if(error){
          console.log("Error is there as  "+ error);
          if(error.error == 603)
          ...
          this.response.write(JSON.stringify(properties));
           this.response.end();
        }
      }); */
    },
